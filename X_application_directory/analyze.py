...
def get_table_schema(sql_query_tool):
    # Define the SQL query to retrieve table and column information
    sql_query = """    
    PRAGMA table_info('legal_questions')
    """
    ...
    for table in tables["name"]:
        ...
        # Output the table's information
        output.append(f"table: {table}, columns: {', '.join(columns)}")
...
class SQL_Query(ChatGPT_Handler):
    def __init__(
        ...
        **kwargs,
        ...
        self.db_path = "data/legal_questions.db"
...
class AnalyzeGPT(ChatGPT_Handler):
    def __init__(
        ...
        **kwargs,
        ...
        table_schema = get_table_schema(sql_query_tool)
        system_message = f"""
        <<data_sources>>
        {table_schema}
