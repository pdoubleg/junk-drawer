...
def insert_research(user_input, introduction, quant_facts, publications, books, prev_ai_research):
    with sqlite3.connect('MASTER.db') as conn:
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO Research (user_input, introduction, quant_facts, publications, books, prev_ai_research)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (user_input, introduction, quant_facts, publications, books, prev_ai_research))
...
def generate_research(userInput):
    ...
    with st.expander("Generative Results", expanded=True):
        ...
        insert_research(userInput, intro['output'], quantFacts['output'], papers['output'], readings['output'], prev_ai_research)
        research_text = [userInput, intro['output'], quantFacts['output'], papers['output'], readings['output'], prev_ai_research]
